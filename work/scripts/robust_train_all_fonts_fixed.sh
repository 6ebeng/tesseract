...
generate_training_data() {
    ...
    # Process ALL fonts (including .TTF extension)
    find "$FONTS_PATH" \( -name "*.ttf" -o -name "*.TTF" \) -type f | while read font; do
        if [ ! -f "$font" ]; then
            continue
        fi
        ...
        # Method 1: Remove number prefix and replace underscores with spaces
        FONT_NAME1=$(echo "$FONT_BASE" | sed 's/^[0-9]*_//' | sed 's/_/ /g')
        # Method 2: Use original name
        FONT_NAME2="$FONT_BASE"
        # Method 3: Remove all underscores
        ...
        # Use simpler progress display for better compatibility
        ...
        # Optional: Limit to first N fonts for testing
        # Uncomment the following lines to process only first 50 fonts for testing
        # if [ $FONT_INDEX -ge 50 ]; then
        #     printf "%sLimiting to first 50 fonts for testing%s\n" "$YELLOW" "$NC"
        #     break
        # fi
    done
