...
# Resolve script directory and use its parent as the work dir
...
# Project root is also the parent of work dir
...
USE_ALL_FONTS=true           # Use ALL fonts by default
# Allow fast validation via env overrides
MAX_ITERATIONS="${MAX_ITERATIONS_OVERRIDE:-10000}"   # More iterations for better training
TARGET_ERROR="${TARGET_ERROR_OVERRIDE:-0.001}"       # Lower error rate for better accuracy
DEBUG_INTERVAL="${DEBUG_INTERVAL_OVERRIDE:-100}"
...
check_dependencies() {
    ...
    for cmd in tesseract text2image lstmtraining combine_tessdata; do
...
clean_environment() {
    ...
    # Try to remove the directory; if Windows denies removal, move it aside
    if [ -d "$GROUND_TRUTH_DIR" ]; then
        rm -rf "$GROUND_TRUTH_DIR" 2>/dev/null || {
            printf "%s⚠ Unable to fully remove old ground-truth (Windows lock). Moving aside...%s\n" "$YELLOW" "$NC"
            mv "$GROUND_TRUTH_DIR" "${GROUND_TRUTH_DIR}.old.$(date +%s)" 2>/dev/null || true
        }
    fi
...
generate_training_data() {
    ...
    # Use the pre-generated font list (or auto-generate if missing)
    FONT_LIST_FILE="${FONT_LIST_OVERRIDE:-$WORK_DIR/font-test/fontlist.txt}"
    if [ ! -f "$FONT_LIST_FILE" ]; then
        printf "%s⚠ Font list not found at: %s%s\n" "$YELLOW" "$FONT_LIST_FILE" "$NC"
        printf "Attempting to generate font list now...\n"
        FONT_LIST_OUTPUT="$FONT_LIST_FILE" "$SCRIPT_DIR/generate_font_list.sh" 2>/dev/null || true
    fi
    if [ ! -f "$FONT_LIST_FILE" ]; then
        printf "%s✗ Unable to generate font list automatically%s\n" "$RED" "$NC"
        printf "Please run: FONT_LIST_OUTPUT=/tmp/fontlist.txt scripts/generate_font_list.sh and retry\n"
        exit 1
    fi

    # Apply optional limit to the number of fonts for faster validation
    if [ -n "$FONT_LIST_LIMIT" ] && [ "$FONT_LIST_LIMIT" -gt 0 ] 2>/dev/null; then
        mkdir -p "$OUTPUT_DIR"
        LIMITED_LIST="$OUTPUT_DIR/fontlist.limited.txt"
        head -n "$FONT_LIST_LIMIT" "$FONT_LIST_FILE" > "$LIMITED_LIST" 2>/dev/null || cp "$FONT_LIST_FILE" "$LIMITED_LIST"
        FONT_LIST_FILE="$LIMITED_LIST"
    fi

    TOTAL_FONTS=$(wc -l < "$FONT_LIST_FILE")

    printf "%sConfiguration:%s\n" "$BOLD" "$NC"
    printf "  Total fonts from list: %s\n" "$TOTAL_FONTS"
    ...
    # Process ALL fonts from the generated list (avoid subshell to keep counters)
    while IFS= read -r FONT_NAME; do
        # Progress indicator
        FONT_INDEX=$((FONT_INDEX + 1))
        PERCENT=$((FONT_INDEX * 100 / TOTAL_FONTS))
        
        # Use the exact font name from the list (may contain spaces)
        FULL_FONT_NAME="$FONT_NAME"
        printf "[%3d%%] Processing font %d/%d: %s\n" \
            "$PERCENT" "$FONT_INDEX" "$TOTAL_FONTS" "$FULL_FONT_NAME"

        # Clean the font name for use in filenames
        CLEAN_NAME=$(printf "%s" "$FONT_NAME" | sed 's/[^a-zA-Z0-9]/_/g' | cut -c1-50)
        
        FONT_SUCCESS=false
        
        # Generate base images with different parameters
        VAR_INDEX=0
        
        # Try with limited variations to avoid overwhelming the system
        for spacing in 0.0; do  # Simplified - just use default spacing
            for exposure in 0; do  # Simplified - just use default exposure
                ...
                if [ -f "${OUTPUT_BASE}.tif" ]; then
                    ...
                    # Create corresponding ground truth text file
                    # Use the same corpus content used to render the image
                    cp "$CORPUS_FILE" "${OUTPUT_BASE}.gt.txt" 2>/dev/null || cat "$CORPUS_FILE" > "${OUTPUT_BASE}.gt.txt"
                    ...
                    if [ "$IMAGEMAGICK_AVAILABLE" = true ]; then
                        ...
                        # Count augmented images and create .gt.txt for each
                        AUG_COUNT=$(ls "${OUTPUT_BASE}"*.tif 2>/dev/null | wc -l)
                        if [ "$AUG_COUNT" -gt 1 ]; then
                            TOTAL_IMAGES=$((TOTAL_IMAGES + AUG_COUNT - 1))
                            for aug_img in "${OUTPUT_BASE}"*.tif; do
                                [ -f "$aug_img" ] || continue
                                aug_base="${aug_img%.tif}"
                                if [ ! -f "${aug_base}.gt.txt" ] && [ -f "${OUTPUT_BASE}.gt.txt" ]; then
                                    cp "${OUTPUT_BASE}.gt.txt" "${aug_base}.gt.txt" 2>/dev/null || true
                                fi
                            done
                        fi
                        for augimg in "${OUTPUT_BASE}"*.tif; do
                            [ -f "$augimg" ] || continue
                            augbase="${augimg%.tif}"
                            [ -f "${augbase}.gt.txt" ] || cp "$CORPUS_FILE" "${augbase}.gt.txt" 2>/dev/null || cat "$CORPUS_FILE" > "${augbase}.gt.txt"
                        done
                    fi
                    break # Success, exit inner loops
                fi
    ...
    if [ $FAILED -gt 0 ]; then
        printf "  %sFailed fonts: %d%s\n" "$YELLOW" "$FAILED" "$NC"
    fi
    
    if [ $SUCCESS -eq 0 ]; then
        printf "%sNo training data generated!%s\n" "$RED" "$NC"
        printf "Please check:\n"
        printf "1. Font files are valid and accessible in WSL\n"
...
generate_lstmf_files() {
    ...
    # Only process base images that have corresponding .box files (augmented images lack valid boxes)
    BASE_LIST=$(ls -1 "$GROUND_TRUTH_DIR"/*.box 2>/dev/null | wc -l)
    if [ "$BASE_LIST" -eq 0 ]; then
        printf "%sNo .box files found! Cannot generate LSTMF.%s\n" "$RED" "$NC"
        exit 1
    fi
    TOTAL_TO_PROCESS=$BASE_LIST
    CURRENT=0
    
    for box_file in "$GROUND_TRUTH_DIR"/*.box; do
        [ -f "$box_file" ] || continue
        base_path="${box_file%.box}"
        base_name=$(basename "$base_path")
        [ -f "${base_path}.tif" ] || continue
    # .gt.txt is not required when using .box with lstm.train
        CURRENT=$((CURRENT + 1))
        PERCENT=$((CURRENT * 100 / TOTAL_TO_PROCESS))
        if [ $((CURRENT % 5)) -eq 0 ]; then
            printf "[%3d%%] Converting %d/%d files...\n" "$PERCENT" "$CURRENT" "$TOTAL_TO_PROCESS"
        fi
        tesseract "${base_path}.tif" "${base_path}" --psm 6 -l ara lstm.train >/dev/null 2>&1 || true
        if [ -f "${base_path}.lstmf" ]; then
            echo "${base_path}.lstmf" >> "$OUTPUT_DIR/robust-lstmf.txt"
...
train_robust_model() {
    ...
    # Extract base LSTM network from the selected base traineddata
    BASE_LSTM="$OUTPUT_DIR/${BASE_MODEL}.lstm"
    combine_tessdata -e "$TESSDATA_PREFIX/${BASE_MODEL}.traineddata" "$BASE_LSTM" >/dev/null 2>&1 || true
    if [ ! -f "$BASE_LSTM" ]; then
        printf "%s✗ Failed to extract base LSTM from %s%s\n" "$RED" "$BASE_MODEL" "$NC"
        exit 1
    fi
...
finalize_model() {
    ...
    # Stop training and create final model using the same base traineddata used for training
    TARGET_TRAINEDDATA="$TESSDATA_PREFIX/${BASE_MODEL}.traineddata"
    lstmtraining \
        --stop_training \
